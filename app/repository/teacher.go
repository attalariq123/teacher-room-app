...
func (u *TeacherRepo) AddTeacher(teacher model.Teacher) error {
	return u.db.Model(model.Teacher{}).Create(&teacher).Error
}

func (u *TeacherRepo) ReadTeacher() ([]model.ViewTeacher, error) {

	var teacherList []model.ViewTeacher
	err := u.db.Table("teachers").Select("name, field_of_study, age").Where("deleted_at is NULL").Find(&teacherList).Error
